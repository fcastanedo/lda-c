...
void save_gamma(char* filename, double** gamma, long num_docs, int num_topics)
{
    FILE* fileptr;
    int k;
    long d;
...
void run_em(char* start, char* directory, corpus* corpus)
{
    ...
    int max_length = corpus->max_length;
    ...
    while (((converged < 0) || (converged > EM_CONVERGED) || (i <= 2)) && (i <= EM_MAX_ITER))
    {
        ...
        for (d = 0; d < corpus->num_docs; d++)
        {
            if ((d % 1000) == 0) printf("document %ld\n",d);
    ...
    for (d = 0; d < corpus->num_docs; d++)
    {
        if ((d % 100) == 0) printf("final e step document %ld\n",d);
