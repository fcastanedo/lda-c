...
lda_suffstats* new_lda_suffstats(lda_model* model)
{
    ...
    long num_terms = model->num_terms;
    long i,j;
...
void corpus_initialize_ss(lda_suffstats* ss, lda_model* model, corpus* c)
{
    ...
    for (k = 0; k < num_topics; k++)
    {
        for (i = 0; i < NUM_INIT; i++)
        {
            ...
            printf("initialized with document %ld\n", d);
            doc = &(c->docs[d]);
...
lda_model* new_lda_model(long num_terms, int num_topics)
{
    long i,j;
    lda_model* model;
